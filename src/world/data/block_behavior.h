...
typedef struct block_behavior {
    ...
    void (*prepare_neighbor_shape)(
        const BlockState* state, Level* level, Vec3i, i32 flags, i32 max_update_depth);
    bool (*pathfindable)(const BlockState* state, enum PathFindingType type);
    const BlockState* (*update_shape)(const BlockState* state,
                                      enum Direction direction,
                                      const BlockState* neighborState,
                                      Level* level,
                                      Vec3i pos,
                                      Vec3i neighborPos);
    bool (*should_skip_rendering)(const BlockState* state,
                                  const BlockState* previous_state,
                                  enum Direction side);
    void (*on_neighbor_update)(const BlockState* state,
                               Level* level,
                               Vec3i pos,
                               const Block* neighbor_block,
                               Vec3i neighbor_pos,
                               bool notify);
    void (*new_state_change)(const BlockState* new_state,
                             Level* level,
                             Vec3i pos,
                             const BlockState* old_state,
                             bool notify);
    void (*old_state_change)(const BlockState* old_state,
                             Level* level,
                             Vec3i pos,
                             const BlockState* new_state,
                             bool moved);
    void (*on_exploded)(const BlockState* state,
                        Level* level,
                        Vec3i pos,
                        const Explosion* explosion,
                        void* stack_merger); // TODO stack merger type
    enum UseResult (*on_use_no_item)(const BlockState* state,
                                     Level* level,
                                     Vec3i pos,
                                     long player_entity_id,
                                     BlockHitResult* hit);
    enum ItemUseResult (*on_use_with_item)(const BlockState* state,
                                           Level* level,
                                           Vec3i pos,
                                           long player_entity_id,
                                           BlockHitResult* hit,
    ...
    // Always returns a single constant in all cases
    enum BlockRenderType (*get_render_type)(const BlockState* state);
