...
*** BROKEN Client ping failure
The client close the connection while pinging, probably due to an invalid response being sent by the server.
...
** DONE Complex data structures
CLOSED: [2025-03-16 dim. 00:48]
*** DONE Refactor the JSON structure & parser
CLOSED: [2025-03-16 dim. 00:48]
- [X] Make dynamic arenas for the json parser

** Memory managment
*** DONE Make object pools not rely on arenas
CLOSED: [2025-09-16 mar. 20:18]
All object pools are in statically allocated memory
*** TODO Dynamic memory blocks for arenas
Arenas should use a system of chained blocks similar to vectors and object pools (before the refactor)
*** DONE Tracked memory blocks
CLOSED: [2025-09-16 mar. 20:18]
Memory blocks should be tracked and managed independently from  memory allocators (arenas, pools...)
Which allocator uses which blocks can be tracked to enhance memory statistics.

** World loading [20%]
*** DONE Block loading
CLOSED: [2025-09-16 mar. 20:21]
- [X] Section palette
- [X] Section palette indices

*** TODO Biome loading
*** TODO Block Entity loading
*** TODO Proto-chunks
*** TODO Structures
...
*** Tooling
- [ ] NBT Explorer / Editor
- [ ] Inspector (Memory stats, network activity, state of game...)
